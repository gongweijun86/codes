...
{
	tsph.sync = TS_SYNC_BYTE(ts_stream);
	if(tsph.sync != 0x47){
		debuglog("sync bytes missed!");
		return FALSE;
	}
	tsph.transport_error_indicator = TS_ERROR(ts_stream);
	tsph.payload_unit_start_indicator = TS_PAYLOAD_START(ts_stream);
	...
	debuglog("
		ts:transport_error_indicator:%d\n
		...
		tsph.payload_unit_start_indicator,
	...
	if(!tsph.transport_error_indicator)
	{
		...
		if(is_psi(tsph.PID))
		{
			...
			{
				BYTE* t=ts_stream + len;
				tsph.pointer_field = TS_POINTER_FIELD(t);
			...
			switch(tsph.adaptation_field_control)
			{
				case 0x03:
					/* 有adaptation,也有净荷 */
					tsph.adaptation_fields = (ADAPT_FILD *)malloc(sizeof(ADAPT_FILD));
...
parse_PMT(BYTE* sec_start)
{
	...
	for(;sin_es_len < all_es_len;)
	{
		...
		pmt.esinfo->reserved2 = PMT_ESINFO_RESERVED2(temp);
		pmt.esinfo->ES_info_length = PMT_ESINFO_LENGTH(temp);

		sin_es_len = 5 + pmt.esinfo->ES_info_length;
...
{
	nit.table_id = NIT_TABLE_ID(sec_start);
	nit.section_syntax_indicator = NIT_SECTION_SYNX(sec_start);
	nit.reserved_future_use1 = NIT_RESERVED_FUTURE_USD1(sec_start);
	nit.reserved1 = NIT_RESERVED1(sec_start);
	nit.section_length = NIT_SECTION_LENGTH(sec_start);
	nit.network_id = NIT_NETWORK_ID(sec_start);
	nit.reserved2 = NIT_RESERVED2(sec_start);
	nit.version_number = NIT_VERSION_NUM(sec_start);
	nit.current_next_indicator = NIT_CURRENT_NEXT_IND(sec_start);
	nit.section_number = NIT_SECTION_NUM(sec_start);
	nit.last_section_number = NIT_LST_SECTION_NUM(sec_start);
	nit.reserved_future_use2 = NIT_RESERVED_FUTURE_USD2(sec_start);
	nit.network_descriptor_length = NIT_NETWORK_DESC_LENGTH(sec_start);
	nit.reserved_future_use3 = NIT_RESERVED_FUTURE_USD3(sec_start);
	nit.transport_stream_loop_length = NIT_TS_LOOP_LENGTH(sec_start);
	nit.CRC = NIT_CRC(sec_start);
...
{
	eit.table_id = EIT_TABLE_ID(sec_start);
	eit.section_syntax_indicator = EIT_SECTION_SYNX(sec_start);
	eit.reserved_future_use1 = EIT_RESERVED_FUTURE_USD1(sec_start);
	eit.reserved1 = EIT_RESERVED1(sec_start);
	eit.section_length = EIT_SECTION_LENGTH(sec_start);
	eit.service_id = EIT_SERVICE_ID(sec_start);
	eit.reserved2 = EIT_RESERVED2(sec_start);
	eit.version_number = EIT_VERSION_NUM(sec_start);
	eit.current_next_indicator = EIT_CURRENT_NEXT_IND(sec_start);
	eit.section_number = EIT_SECTION_NUM(sec_start);
	eit.last_section_number = EIT_LST_SECTION_NUM(sec_start);
	eit.transport_stream_id = EIT_TS_ID(sec_start);
	eit.original_network_id = EIT_ORIGINAL_NETWORK_ID(sec_start);
	eit.segment_last_section_number = EIT_SEG_LST_SECTION_NUM(sec_start);
	eit.last_table_id = EIT_LST_TABLE_ID(sec_start);
	eit.CRC_32 = EIT_CRC(sec_start);

	debuglog("eit.table_id:%d\n	
		eit.section_syntax_indicator:%d\n	
		eit.reserved_future_use1:%d\n	
		eit.reserved1:%d\n	
		eit.section_length:%l\n	
		eit.service_id:%l\n	
		eit.reserved2:%d\n	
		eit.version_number:%d\n	
		eit.current_next_indicator:%d\n	
		eit.section_number:%d\n	
		eit.last_section_number:%d\n	
		eit.transport_stream_id:%l\n	
		eit.original_network_id:%l\n	
		eit.segment_last_section_number:%d\n	
		eit.last_table_id:%d\n	
		eit.CRC_32:%d\n",	
		eit.table_id,
		eit.section_syntax_indicator,
		eit.reserved_future_use1,
		eit.reserved1,
		eit.section_length,
		eit.service_id,
		eit.reserved2,
	...
	BYTE* start = sec_start + 14;
	BYTE* end = start + eit.section_length - 11 - 4;
	BYTE* temp = start;
	for(;start <= end;)
	{
		eit.description = (EIT_DESC *)malloc(sizeof(EIT_DESC));
		eit.description->event_id = *(temp);
		eit.description->start_time = *(++temp);
		eit.description->duration = *(++temp);
		eit.description->running_status = *(++temp);
		eit.description->free_CA_mode = *(++temp);
		eit.description->descriptors_loop_length = *(++temp);
		eit.description->descriptor = malloc(sizeof(BYTE) * eit.description->descriptors_loop_length);
		
		int l = 1;
		for(;l < eit.description->descriptors_loop_length;)
		{
			switch(*(temp + l))
			{
				case 0x4d:
					...
					h->desc_len = *(temp + l);
					l++;
					h->language = (*(temp + l) << 16) || (*(temp + l + 1) << 8) || (*(temp + l + 2));
					l+=3;
					h->event_name_len = *(temp + l);
					l++;
					h->event_name = (char *)malloc(sizeof(char) * h->event_name_len);
					memcpy(h->event_name,temp + l,h->event_name_len);
					l+=h->event_name_len;
					h->text_len = *(temp + l);
					l++;
					h->text = (char *)malloc(sizeof(char) * h->text_len);
					memcpt(h->text,temp + l,h->text_len);
					l+=h->text_len;
					break;
				default:
					break;
