 ...
 *       Filename:  pmt.h
 *
 *    Description:  Programe Map Table definition
 *
 *        Version:  1.0
 *        Created:  2011年06月02日 13时53分52秒
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  oiron 
...
#define PMT_RESERVED3(b)			((b[8] & 0x70) >> 5)
#define PMT_PCR_PID(b)				((b[8] & 0x1f) << 8 | b[9])
#define PMT_RESERVED4(b)			((b[10] & 0xf0) >> 4)
#define PMT_PROGRAM_INFO_LENGTH(b)		((b[10] & 0x0f) << 8 | b[11])

#define PMT_ESINFO_STREAM_TYPE(b)		({ int n = PMT_PROGRAM_INFO_LENGTH(b); \
						   int base = 12 + n; \
						   b[base]; \
						    })

#define	PMT_ESINFO_RESERVED1(b)			({ int n = PMT_PROGRAM_INFO_LENGTH(b); \
						   int base = 13 + n; \
						   (b[base] & 0x70) >> 5; \
						    })		/*  */

#define	PMT_ESINFO_ELEPID(b)			({ int n = PMT_PROGRAM_INFO_LENGTH(b); \
						   int base = 13 + n; \
						    (b[base] & 0x1f) << 8 | b[base + 1]; \
						    })/*  */

#define PMT_ESINFO_RESERVED2(b)			({ int n = PMT_PROGRAM_INFO_LENGTH(b); \
						   int base = 15 + n; \
						    (b[base] & 0xf0) >> 4; \
						    })				/*  */

#define	PMT_ESINFO_LENGTH(b)			({ int n = PMT_PROGRAM_INFO_LENGTH(b); \
						   int base = 15 + n; \
